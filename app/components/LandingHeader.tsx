...
export default function LandingHeader() {
  ...
  return (
    <>
      {/* Reusable glass pill wrapper */}
      {(() => {
        const hideTransform = `translateY(calc(-100% - ${HEADER.topOffset}px))`;

        // Layers that make up the GlassPill look (no mouse-tracking subsurface scatter)
        const GlassLayers = () => (
          <>
            {/* Lens distortion */}
            ...
            <div style={{ position: 'absolute', inset: 0, borderRadius: 9999, pointerEvents: 'none', overflow: 'hidden', filter: 'blur(8px)', opacity: 0.20 }}>
            ...
            {/* Base glass body */}
            <div style={{ position: 'absolute', inset: 0, borderRadius: 9999, background: 'rgba(255,255,255,0.50)', backdropFilter: 'blur(60px)', WebkitBackdropFilter: 'blur(60px)', border: '1.5px solid rgba(255,255,255,0.90)', boxShadow: 'inset 0 12px 24px rgba(255,255,255,1), inset 0 -10px 20px rgba(0,0,0,0.05), 0 10px 30px -10px rgba(0,0,0,0.10)' }} />
            {/* Fresnel top rim */}
            <div style={{ position: 'absolute', inset: 0.5, borderRadius: 9999, borderTop: '1.5px solid white', pointerEvents: 'none', zIndex: 20 }} />
            {/* Surface glare */}
            <div style={{ position: 'absolute', top: 1.2, left: '8%', width: '84%', height: '40%', borderRadius: '100% 100% 0 0', background: 'linear-gradient(to bottom, rgba(255,255,255,0.95), transparent)', pointerEvents: 'none', zIndex: 20 }} />
          </>
        );
        ...
        return (
          /* Centered row — both pills sit inside a max-width container */
          <div style={{
